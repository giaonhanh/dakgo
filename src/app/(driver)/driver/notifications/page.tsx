      ...
      <div style={{ background:"rgba(8,8,6,0.96)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,107,0,0.08)", paddingTop:"env(safe-area-inset-top)", flexShrink:0 }}>
        <div style={{ height:56, padding:"0 16px", display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ color:"#f8f0e0", fontSize:14, fontWeight:800, display:"flex", alignItems:"center", gap:6 }}>
              Thông báo
              {unreadCnt > 0 && (
        ...
        {loading ? (
          ...
          return (
            <div key={n.id}
              ...
              {/* Card */}
              <div
                onClick={() => { if (swiped) { setSwipedId(null); return } if (!n.is_read) markRead(n.id) }}
                style={{ display:"flex", gap:12, alignItems:"flex-start", padding:"12px 14px", borderRadius:14,
                  cursor:"pointer",
