...
export default function MerchantProfilePage() {
    ...
    async function load() {
      try {
        ...
        if (shop) {
          ...
          const oh = shop.opening_hours as { open?: string; close?: string; days?: string[] } | null
          if (oh?.open)  setOpenTime(oh.open)
          if (oh?.close) setCloseTime(oh.close)
          if (oh?.days)  setDays(oh.days)
        }
      } catch { /* ignore */ }
      setLoading(false)
    }
  ...
  const handleSave = async () => {
    ...
    if (shopId) {
      const { error } = await supabase.from("shops").update({
        name, phone, address,
        opening_hours: { open: openTime, close: closeTime, days },
  ...
  const handlePwNext = async () => {
    ...
    if (pwStep === 1) {
      ...
      } catch { setPwErr("Có lỗi xảy ra, vui lòng thử lại") }
  ...
  return (
    <>
      ...
      {/* Toast stack */}
      <div style={{ position:"fixed",top:60,left:16,right:16,zIndex:200,display:"flex",flexDirection:"column",gap:8,pointerEvents:"none" }}>
        {toasts.map(t => (
      ...
      <div style={{ position:"fixed",inset:0,background:"#080806",display:"flex",flexDirection:"column",overflow:"hidden" }}>
        ...
        <div style={{ flex:1,overflowY:"auto",padding:"16px 16px 40px" }}>
            ...
            <div style={{ display:"flex",gap:5,flexWrap:"wrap",marginBottom:10 }}>
              {ALL_DAYS.map(d => (
                <button key={d} onClick={editing ? () => toggleDay(d) : undefined}
                  style={{ width:34,height:34,borderRadius:8, background:days.includes(d)?"rgba(255,107,0,0.12)":"rgba(255,255,255,0.04)", border:days.includes(d)?"1px solid rgba(255,107,0,0.35)":"1px solid rgba(255,255,255,0.06)", display:"flex",alignItems:"center",justifyContent:"center", color:days.includes(d)?"#FF8C00":"#6a5a40", fontSize:10,fontWeight:700, cursor:editing?"pointer":"default" }}>
                  {d}
                </button>
              ))}
            </div>
              ...
              {editing ? (
                <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                  <input type="time" value={openTime} onChange={e => setOpenTime(e.target.value)}
                    style={{ background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,107,0,0.3)",borderRadius:8,padding:"3px 8px",color:"#f8f0e0",fontSize:11,fontWeight:700,colorScheme:"dark" } as React.CSSProperties} />
                  <span style={{ color:"#6a5a40" }}>–</span>
                  <input type="time" value={closeTime} onChange={e => setCloseTime(e.target.value)}
                    style={{ background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,107,0,0.3)",borderRadius:8,padding:"3px 8px",color:"#f8f0e0",fontSize:11,fontWeight:700,colorScheme:"dark" } as React.CSSProperties} />
                </div>
              ) : (
                <span style={{ color:"#f8f0e0",fontSize:11,fontWeight:700 }}>{openTime} – {closeTime}</span>
              )}
            </div>
          </div>
          ...
          <button onClick={openPwSheet}
            style={{ width:"100%",display:"flex",alignItems:"center",gap:12,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:14,marginBottom:8,textAlign:"left",cursor:"pointer" }}>
            <div style={{ width:40,height:40,borderRadius:11,background:"rgba(74,143,245,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20 }}>🔑</div>
            <div style={{ flex:1 }}>
          ...
          <button onClick={() => setShowLogoutConfirm(true)}
            style={{ width:"100%",height:48,borderRadius:14,background:"rgba(255,64,64,0.08)",border:"1px solid rgba(255,64,64,0.2)",color:"#ff4040",fontSize:12,fontWeight:700,cursor:"pointer",marginTop:8 }}>
            🚪 Đăng xuất
          </button>
        </div>
        ...
        {showLogoutConfirm && (
          <>
            <div onClick={() => setShowLogoutConfirm(false)} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:50,backdropFilter:"blur(4px)" }} />
            <div style={{ position:"fixed",bottom:0,left:0,right:0,background:"#0e0c09",borderRadius:"20px 20px 0 0",border:"1px solid rgba(255,255,255,0.08)",padding:"24px 16px 36px",zIndex:51 }}>
              <div style={{ textAlign:"center",marginBottom:20 }}>
              ...
              <div style={{ display:"flex",gap:8 }}>
                <button onClick={() => setShowLogoutConfirm(false)} style={{ flex:1,height:48,borderRadius:12,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.08)",color:"#b0956a",fontSize:12,fontWeight:700,cursor:"pointer" }}>Hủy</button>
                <button onClick={handleLogout} style={{ flex:1,height:48,borderRadius:12,background:"rgba(255,64,64,0.12)",border:"1px solid rgba(255,64,64,0.3)",color:"#ff4040",fontSize:12,fontWeight:700,cursor:"pointer" }}>🚪 Đăng xuất</button>
              </div>
            </div>
          </>
        )}
        ...
        {showPwSheet && (
          <>
            <div onClick={() => setShowPwSheet(false)} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:50,backdropFilter:"blur(4px)" }} />
            <div style={{ position:"fixed",bottom:0,left:0,right:0,background:"#0e0c09",borderRadius:"22px 22px 0 0",border:"1px solid rgba(74,143,245,0.2)",padding:"20px 18px 36px",zIndex:51 }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:6,justifyContent:"center",marginBottom:18 }}>
                {([1,2,3] as const).map(s => (
                  <div key={s} style={{ display:"flex",alignItems:"center",gap:6 }}>
                    <div style={{ width:26,height:26,borderRadius:13,background:pwStep>=s?"rgba(74,143,245,0.2)":"rgba(255,255,255,0.05)",border:`1px solid ${pwStep>=s?"rgba(74,143,245,0.5)":"rgba(255,255,255,0.08)"}`,display:"flex",alignItems:"center",justifyContent:"center",color:pwStep>=s?"#4a8ff5":"#6a5a40",fontSize:10,fontWeight:700 }}>
                      {pwStep > s ? "✓" : s}
                    </div>
              ...
              <input type="password"
                ...
                onChange={e => { setPwErr(""); if(pwStep===1)setPwOld(e.target.value); else if(pwStep===2)setPwNew(e.target.value); else setPwCfm(e.target.value) }}
                placeholder={pwStep===1?"Mật khẩu hiện tại":pwStep===2?"Mật khẩu mới":"Nhập lại mật khẩu mới"}
                style={{ width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${pwErr?"rgba(255,64,64,0.5)":"rgba(74,143,245,0.3)"}`,borderRadius:12,padding:"12px 14px",color:"#f8f0e0",fontSize:14,fontFamily:"Lexend",marginBottom:6,boxSizing:"border-box" }} />
              {pwErr && <div style={{ color:"#ff4040",fontSize:10,marginBottom:10,textAlign:"center" }}>⚠ {pwErr}</div>}
              <div style={{ display:"flex",gap:8,marginTop:8 }}>
                <button onClick={() => { if(pwStep>1)setPwStep(s=>(s-1) as 1|2|3); else setShowPwSheet(false) }}
                  style={{ flex:1,height:46,borderRadius:12,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",color:"#b0956a",fontSize:12,fontWeight:700,cursor:"pointer" }}>
                  {pwStep===1?"Huỷ":"← Quay lại"}
                </button>
                <button onClick={handlePwNext} disabled={pwLoading}
                  style={{ flex:2,height:46,borderRadius:12,border:"none",background:"linear-gradient(90deg,#4a8ff5,#6a9ff5)",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"Lexend",opacity:pwLoading?0.7:1 }}>
