    ...
    async function load() {
      ...
      try {
        ...
        const { data: orders } = await supabase
          .from("orders")
          .select("id,total_amount,ship_fee,pay_method,created_at,total,discount_amount")
          .eq("shop_id", shop.id)
          .eq("status", "delivered")
        ...
        if (period === "week") {
          ...
          for (let i = 6; i >= 0; i--) {
            ...
            days.push({
              day: label,
              subtotal: dayOrders.reduce((s, o) => s + (o.total ?? o.total_amount ?? 0), 0),
          ...
          for (let w = 0; w < 4; w++) {
            ...
            if (wOrders.length > 0 || w < Math.ceil(now.getDate() / 7)) {
              weeks.push({
                day: `T${w+1}`,
                subtotal: wOrders.reduce((s, o) => s + (o.total ?? o.total_amount ?? 0), 0),
        ...
        if (todayOrders.length > 0) {
          ...
          setRecentOrders(todayOrders.map(o => {
            ...
            return {
              ...
              items: summary, subtotal: o.total ?? o.total_amount ?? 0,
