...
export default function ErrandPage() {
  const [tab,     setTab]     = useState<"buy" | "deliver">("buy")
    ...
    <>
      ...
      <div style={{ position:"fixed",inset:0,background:"#080806",display:"flex",flexDirection:"column",overflow:"hidden" }}>
                  ...
                  {items.map((item, idx) => (
                    <div key={item.id} style={{ display:"flex",gap:7,marginBottom:8,alignItems:"center" }}>
                      ...
                      <div style={{ display:"flex",alignItems:"center",gap:3,flexShrink:0 }}>
                        <button onClick={()=>updateItem(item.id,"qty",Math.max(1,item.qty-1))} style={{ width:24,height:24,borderRadius:6,background:"rgba(255,255,255,0.06)",border:"none",color:"#f8f0e0",fontSize:13,cursor:"pointer" }}>−</button>
                        <span style={{ color:"#f8f0e0",fontSize:11,width:14,textAlign:"center" }}>{item.qty}</span>
                        <button onClick={()=>updateItem(item.id,"qty",item.qty+1)} style={{ width:24,height:24,borderRadius:6,background:"rgba(255,255,255,0.06)",border:"none",color:"#f8f0e0",fontSize:13,cursor:"pointer" }}>+</button>
                      </div>
                      {items.length > 1 && (
        ...
        <div style={{ position:"absolute",bottom:0,left:0,right:0,background:"rgba(8,8,6,0.97)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,0.07)",padding:"12px 16px 28px",zIndex:10 }}>
          <button style={{ width:"100%",height:52,borderRadius:14,background:"linear-gradient(90deg,#FF6B00,#FF8C00,#FFB347)",border:"none",cursor:"pointer",color:"#fff",fontSize:14,fontWeight:800,fontFamily:"Lexend",boxShadow:"0 4px 24px rgba(255,107,0,0.45)",position:"relative",overflow:"hidden" }}>
            ...
            <span style={{ position:"relative",zIndex:1 }}>{tab==="buy"?"🛍️ Đặt mua hộ":"📦 Đặt giao hộ"}</span>
          </button>
        </div>
      </div>
