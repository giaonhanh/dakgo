...
// BUG-008 fix: "ride" matches DB enum notif_type (was "driver")
type NotifType = "order" | "promo" | "system" | "ride"
...
// BUG-008 fix: key "ride" matches DB enum, was "driver"
const TABS = [
  { key: "all",    label: "Tất cả",     icon: "🔔" },
  { key: "order",  label: "Đơn hàng",   icon: "📦" },
  { key: "promo",  label: "Khuyến mãi", icon: "🎁" },
  { key: "ride",   label: "Tài xế",     icon: "🛵" },
...
export default function NotificationsPage() {
  ...
  // Initial load + BUG-005: Realtime subscription
  useEffect(() => {
    load()
  ...
  // BUG-007 fix: optimistic update with rollback on error
  const markRead = async (id: string) => {
    setNotifs(prev => prev.map(n => n.id === id ? { ...n, isRead: true } : n))
    const { error } = await supabase.from("notifications").update({ is_read: true }).eq("id", id)
    if (error) setNotifs(prev => prev.map(n => n.id === id ? { ...n, isRead: false } : n))
  }

  // BUG-006 fix: use DB query instead of captured IDs to avoid race condition
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", inset: 0, background: "#080806", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <button onClick={() => router.back()}
              style={{ width: 40, height: 40, borderRadius: 9, cursor: "pointer",
                ...
                display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: "#f8f0e0" }}>←</button>
            <div style={{ flex: 1 }}>
          ...
          {loading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column",
              ...
              <button onClick={() => router.push("/")}
                style={{ marginTop: 8, padding: "8px 20px", borderRadius: 10, border: "none", cursor: "pointer",
                  background: "rgba(255,107,0,0.1)", color: "#FF8C00", fontSize: 12, fontWeight: 600 }}>
                Đặt hàng ngay →
              </button>
            </div>
          ) : (
            <AnimatePresence>
              {filtered.map((n, idx) => (
                <motion.div key={n.id}
                  ...
                  {/* BUG fix: use div+onClick instead of <a href> to avoid full reload */}
                  <div onClick={() => { markRead(n.id); router.push(n.href) }} style={{ cursor: "pointer" }}>
                    <div style={{
                      ...
                      <div style={{ width: 38, height: 38, borderRadius: 11, background: n.iconBg,
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 10, fontWeight: 600, padding: "1px 6px", borderRadius: 5,
                            background: n.type === "order" ? "rgba(255,107,0,0.1)" :
                                        ...
                                        n.type === "ride"  ? "rgba(62,207,110,0.1)" : "rgba(74,143,245,0.1)",
                            color: n.type === "order" ? "#FF8C00" :
                                   n.type === "promo" ? "#FFB347" :
                                   n.type === "ride"  ? "#3ecf6e" : "#4a8ff5",
                          }}>
                            {n.type === "order" ? "Đơn hàng" :
                             n.type === "promo" ? "Khuyến mãi" :
                             n.type === "ride"  ? "Tài xế" : "Hệ thống"}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  {!n.isRead && (
                    <div style={{ position: "absolute", top: 9, right: 22,
                      width: 7, height: 7, borderRadius: "50%",
                      background: "#FF6B00", boxShadow: "0 0 5px rgba(255,107,0,0.6)",
        ...
        {/* Bottom Nav */}
        <div style={{ position: "absolute", bottom: "max(16px,env(safe-area-inset-bottom))",
          left: 14, right: 14, height: 56,
