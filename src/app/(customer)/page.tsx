...
// Trang ch? — 12 Sections Ð?y Ð? theo mockup dã approved
// S0  HomeHeader       — GPS Radar + Bell + Avatar
// S1  AIGreeting       — Chào theo gi? + AI g?i ý
// S2  SearchBar        — Tìm ki?m + Filter
// S3  LiveStatusBanner — Ðon dang giao (hi?n có di?u ki?n)
// S4  FlashSaleBanner  — Banner khuy?n mãi + countdown
// S5  ServiceGrid      — 4 d?ch v? nhanh
// S6  VoucherStrip     — Voucher s?p h?t h?n
// S7  CategoryCarousel — L?c lo?i món an
// S8  PromoSection     — Khuy?n mãi hôm nay
// S9  NearbyShops      — Quán g?n b?n
// S10 BestSellers      — Bán ch?y tu?n này
// S11 LoyaltyPoints    — Ði?m tích luy
// S12 ReorderSection   — Ð?t l?i nhanh
...
// Danh m?c hi?n th? trên trang ch? — l?y t? SHOP_CATEGORIES (b? "Khác")
const HOME_CATS = SHOP_CATEGORIES.filter(c => c.value !== "khac")

// --- Không còn mock data — dùng Supabase th?t -------------


// --- Helpers ------------------------------------------------
function shopInHoursFromHours(oh: { open?: string; close?: string } | null | undefined): boolean {
  if (!oh?.open || !oh?.close) return true  // không có gi? ? không gi?i h?n
  const now = new Date()
...
// Tính quán có dang trong gi? m? c?a không (múi gi? VN UTC+7)
function isShopInHours(shop: ShopRow): boolean {
  ...
  // Format m?i: DayHours[] — m?i ngày có slots riêng
  if (Array.isArray(oh)) {
...
function getWeatherTip(code: number, temp: number, hour: number): string {
  if (code >= 95) return "?? Bão giông dang d?n! ? nhà an toàn, order ngay v? thôi!"
  if (code >= 80) return "??? Ðang có mua rào — d?t d? an giao v?, kh?i u?t!"
  if (code >= 51) return "? Tr?i mua r?i, d?ng ra ngoài — order v? nhà ?m cúng hon!"
  if (code >= 45) return "??? Suong mù dày, h?n ch? di chuy?n — d?t v? nhà nhé!"
  if (temp >= 35) return `??? Nóng ${Math.round(temp)}°C r?i! Sinh t?, nu?c ép l?nh gi?i nhi?t ngay!`
  if (temp >= 30) return `?? Tr?i ${Math.round(temp)}°C — trà dá, trà s?a dá cho mát nhé!`
  if (temp <= 20) return `?? Mát ${Math.round(temp)}°C — bún bò, ph? nóng h?p th?i ti?t l?m!`
  if (hour < 10) return "? Sáng mát, u?ng cà phê hay an bánh mì nóng nhé!"
  if (hour < 12) return "? G?n trua r?i, d?t com tru?c d? không ch? lâu!"
  if (hour < 14) return "?? Gi? com trua — d?t ngay k?o h?t su?t nhé!"
  if (hour < 18) return "?? Chi?u mát, u?ng gì cho t?nh ngu?i di nào!"
  return "?? T?i r?i, bún bò hay cháo an là ngon nh?t!"
}
...
export default function HomePage() {
  ...
  // Ð?c d?a ch? t? locationStore (dã du?c GpsInit trong layout l?y s?n)
  const locationData = useLocationStore()
  const location = locationData.address || "Phu?c An, Krông P?c"
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Live orders: don d? an dang x? lý
      ...
      // Live rides (xe ôm / taxi dang tìm xe / dang di)
      ...
      // Live errands (giao h? / mua h? dang x? lý)
      ...
      const ridesMapped = (liveRides ?? []).map(r => ({
        ...
        shops: { name: r.vehicle_type === "motorbike" ? "?? Xe ôm" : r.vehicle_type === "car_7" ? "?? Taxi 7 ch?" : "?? Taxi 4 ch?" },
        _href: "/orders",
        _type: "ride" as const,
      }))
      ...
      // Nearby shops: fetch c? dóng l?n m?, tính gi? client-side
      const { data: shopData } = await supabase
        ...
        .limit(30)
      // Sort: dang m? lên tru?c, dóng xu?ng du?i
      ...
      // Combo vouchers dang active — dánh d?u quán nào có combo
      ...
      // Best sellers — top bán ch?y, không l?c theo gi? (sold_count >= 0)
      ...
      // Promos — s?n ph?m có giá khuy?n mãi (original_price > price)
      const { data: promoData } = await supabase
        ...
        .order("sold_count", { ascending: false })
        .limit(20)
      // Fallback: n?u không có s?n ph?m KM, l?y s?n ph?m bán ch?y nh?t
      ...
      if (promoFiltered.length > 0) {
        ...
        // fallback: top s?n ph?m t? quán dang m?
      ...
      // V?a lên menu — quán dang m?, trong khung gi? bán
  ...
  // Weather tip — dùng t?a d? t? locationStore (GPS dã du?c layout l?y s?n)
  useEffect(() => {
  ...
  const greet = () => {
    ...
    if (h < 12) return "Bu?i sáng t?t lành"
    if (h < 18) return "Bu?i chi?u t?t lành"
    return "Bu?i t?i t?t lành"
  }

  const aiTip = () => {
    const h = new Date().getHours()
    if (h < 10) return "? Sáng mát, u?ng cà phê hay an bánh mì nóng nhé!"
    if (h < 12) return "? G?n trua r?i, d?t com tru?c d? không ch? lâu!"
    if (h < 14) return "?? N?ng nóng, b? sung nu?c — trà s?a ho?c sinh t??"
    if (h < 18) return "?? Bu?i chi?u, an nh? ho?c u?ng trà s?a di!"
    return "?? T?i r?i, bún bò hay cháo an là ngon nh?t!"
  }
      ...
      <div ref={containerRef} style={{
        ...
        <div style={{ flex:1, overflowY:"auto", overflowX:"hidden",
          ...
          <div style={{ padding:"8px 16px 6px", display:"flex",
            ...
            <div style={{ display:"flex", alignItems:"center", gap:7 }}>
              ...
              <div>
                <div style={{ color:"#6a5a40", fontSize: 11 }}>V? trí c?a b?n</div>
          ...
          <div style={{ padding:"2px 16px 12px" }}>
            ...
            <div style={{
              ...
              <div style={{ color:"#b464ff", fontSize: 11, lineHeight:1.4, flex:1 }}>
                <strong style={{ color:"#c87aff" }}>G?i ý AI:</strong>{" "}
                {weatherTip ?? aiTip()}
              </div>
          ...
          <div style={{ margin:"0 16px 12px",
            ...
            <input readOnly placeholder="Tìm món an, c?a hàng, d?ch v?..."
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                      ...
                      <span style={{ color:"#3ecf6e", fontSize: 11, fontWeight:600 }}>
                        {liveOrders.length} don dang x? lý
                      </span>
                    </div>
                    {/* Dots */}
                ...
                <div style={{ overflow:"hidden", borderRadius:14 }}>
                  <div style={{
                    ...
                    {liveOrders.map(order => {
                      const shopName = (order.shops as {name:string}|null)?.name ?? "Quán dang chu?n b?"
                      const isRide   = order._type === "ride"
                      const isErrand = order._type === "errand"
                      const statusLabel = isRide
                        ? (order.status === "pending" ? "Ðang tìm tài x?..." :
                           order.status === "accepted" ? "Tài x? dang d?n" :
                           order.status === "delivering" ? "Ðang trên du?ng" : "Ðang x? lý")
                        : isErrand
                        ? (order.status === "pending" ? "Ðang tìm tài x?..." :
                           order.status === "accepted" ? "Tài x? dang x? lý" :
                           order.status === "delivering" ? "Ðang giao" : "Ðang x? lý")
                        : (order.status === "pending"    ? "Ch? quán xác nh?n" :
                           order.status === "accepted" || order.status === "preparing" ? "Ðã xác nh?n · Ðang làm" :
                           order.status === "ready"      ? "Ðang tìm tài x?" :
                           order.status === "delivering" ? "Ðang giao hàng" : "Ðang x? lý")
                      const statusColor =
          ...
          {vouchers.length > 0 ? (() => {
            ...
            const dealSubLine = deal.discount_type === "percent"  ? `Gi?m ${deal.discount_value}% · Áp d?ng ngay`
              : deal.discount_type === "fixed"    ? `Gi?m ${fmt(deal.discount_value)} · Ð?t ngay`
              : "Mi?n phí giao hàng · Ðon t? b?t k?"
            return (
              ...
              <div style={{ height:110, borderRadius:16, overflow:"hidden",
                ...
                <div style={{ position:"relative", zIndex:1, padding:"13px 15px" }}>
                  ...
                  <div style={{ color:"#fff", fontSize:13, fontWeight:700, lineHeight:1.3, maxWidth:"62%", wordBreak:"break-word" }}>
                    M?i b?n bè, nh?n 5.000 XU!
                  </div>
                  <div style={{ color:"rgba(255,255,255,0.45)", fontSize: 11, marginTop:3 }}>
                    C? hai nh?n 5.000 xu · Ðon d?u t? 50.000d
                  </div>
          ...
          {vouchers.length === 0 ? (
            <div style={{ margin:"0 16px 14px",
              ...
              <div style={{ textAlign:"center" }}>
                ...
                <div style={{ color:"#6a5a40", fontSize:10, lineHeight:1.7 }}>
                  Ð?t don d?u tiên d? nh?n ngay<br/>
                  <span style={{ color:"#FFB347", fontWeight:600 }}>uu dãi h?p d?n t? Giao Nhanh!</span>
                </div>
              </div>
            ...
            <HScroll>
              {vouchers.map(v => {
                ...
                const expiryLabel = daysLeft <= 0 ? "H?t h?n HÔM NAY!" : daysLeft === 1 ? "Còn 1 ngày" : `Còn ${daysLeft} ngày`
                    ...
                    {v.min_order && v.min_order > 0 && (
                      <div style={{ marginBottom:6 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                          <span style={{ fontSize: 10, color:"#6a5a40" }}>Ð?t t?</span>
          ...
          -------------------------------------- */}
          {promos.length > 0 && (<>
          <SectionHeader title="?? Khuy?n mãi hôm nay" more="Xem t?t c? ?" href="/promo-items" />
            <HScroll>
            {promos.map(p => {
                ...
                <div className="promo-card" style={{
                  ...
                  <div style={{ padding:"7px 9px 8px" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center",
                      ...
                      <span style={{ color:"#6a5a40", fontSize:9 }}>?? {p.sold_count} dã bán</span>
                      <button
          ...
          {/* --------------------------------------
              S8.5 — C?a hàng yêu thích
          -------------------------------------- */}
          {favoriteShops.length > 0 && (
            <>
              <SectionHeader title="?? C?a hàng yêu thích" />
              <HScroll>
                {favoriteShops.map(s => (
                    ...
                    <div style={{
                      ...
                      <div style={{ padding:"8px 9px" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:5, marginTop:3 }}>
                          <Badge layer={3} variant={s.is_open ? "open" : "closed"} size="sm" label={s.is_open ? "M?" : "Ðóng"} />
          ...
          {/* --------------------------------------
              S9 — NearbyShops
          -------------------------------------- */}
          <SectionHeader title="?? Quán g?n b?n" more="Xem t?t c? ?" href="/nearby-shops" />

          {/* Filter chips — danh m?c có quán */}
          {(() => {
          ...
          <div style={{ padding:"0 16px", display:"flex", flexDirection:"column",
            ...
            {filteredShops.length === 0 ? (
              <div style={{ textAlign:"center", padding:"20px 0", color:"#6a5a40", fontSize:11 }}>
                Chua có quán nào{nearbyFilter !== "all" ? " trong danh m?c này" : " trong khu v?c"}
              </div>
            ) : filteredShops.map(s => {
              ...
              <div key={s.id} style={{ position:"relative", opacity: shopOpen ? 1 : 0.55 }}>
                {/* Quán dóng: ch?n click, hi?n toast */}
                {!shopOpen && (
                  <div onClick={() => {
                ...
                <a href={shopOpen ? `/shop/${s.id}` : "#"} onClick={e => !shopOpen && e.preventDefault()} style={{ textDecoration:"none" }}>
                  <div className="shop-card" style={{
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      {/* ? rating + ?? km + tr?ng thái — cùng 1 hàng */}
                      <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
                        ...
                        {/* Open/closed */}
                        {shopOpen
                          ? <Badge layer={3} variant="open" size="sm" label="Ðang m?" />
          ...
          {/* --------------------------------------
              S9.5 — V?a lên menu (s?n ph?m m?i nh?t)
          -------------------------------------- */}
          {newMenuItems.length > 0 && (
            <>
              <SectionHeader title="?? V?a lên menu" more="Xem thêm ?" href="/search?sort=newest" />
              <HScroll>
                {newMenuItems.map(p => {
                  const shopName = (p.shops as {name:string}|null)?.name ?? ""
                  return (
          ...
          {/* --------------------------------------
              S9.6 — Món an g?i ý (theo l?ch s? tìm ki?m)
          -------------------------------------- */}
          {searchSuggest.length > 0 && (
            <>
              <SectionHeader title="?? Món an g?i ý" more="Tìm ki?m ?" href="/search" />
              <HScroll>
                {searchSuggest.map(p => {
                  const shopName = (p.shops as {name:string}|null)?.name ?? ""
                  return (
                    <div key={p.id} className="promo-card" style={{
                      ...
                      <div style={{ padding:"7px 9px 8px" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center",
                          justifyContent:"space-between", marginTop:4 }}>
                          <span style={{ color:"#6a5a40", fontSize: 10 }}>?? {p.sold_count} dã bán</span>
                          <button
          ...
          {recos.length > 0 && (
            <>
              <SectionHeader title="? G?i ý cho b?n" more="Xem thêm ?" href="/search" />
              <HScroll>
                {recos.map(p => (
                  <a key={p.id} href={`/shop/${p.shop_id}`}
          ...
          <SectionHeader title="?? Bán ch?y tu?n này" more="Xem t?t c? ?" href="/bestsellers" />
            <HScroll>
                  ...
                  <div key={b.id} style={{
                    ...
                    <div style={{ padding:"7px 9px 8px" }}>
                      ...
                      <div style={{ color:"#3ecf6e", fontSize: 10, fontWeight:600, marginTop:3 }}>
                        ?? {b.sold_count.toLocaleString("vi-VN")} dã bán
                      </div>
          ...
          {/* S11 — LoyaltyPoints removed: di?m ch? hi?n th? trong Profile cá nhân */}

          {/* S11 — C?a hàng yêu thích: s? hi?n khi có b?ng favorites */}

          {/* --------------------------------------
              S12 — ReorderSection
          -------------------------------------- */}
          <SectionHeader title="?? Ð?t l?i nhanh" more="L?ch s? ?" href="/orders" />
          {reorders.length === 0 ? (
            <div style={{ padding:"0 16px 14px" }}>
              <div style={{ background:"rgba(255,255,255,0.03)", border:"1px dashed rgba(255,255,255,0.07)",
                borderRadius:12, padding:"16px", textAlign:"center" }}>
                <div style={{ fontSize:28, marginBottom:6 }}>???</div>
                <div style={{ color:"#6a5a40", fontSize:10 }}>Ð?t don d?u tiên d? th?y<br/>l?ch s? d?t l?i nhanh ? dây</div>
              </div>
            </div>
          ) : (
            <HScroll>
              {reorders.map(r => {
                ...
                const firstItem = (r.order_items as {name:string}[])?.[0]?.name ?? "Ðon hàng"
                return (
                  <div key={r.id} style={{
                    ...
                    <a href={`/shop/${r.shop_id}`} style={{ textDecoration:"none" }}>
                      <div className="reorder-btn" style={{
                        ...
                        ?? Ð?t l?i · {Math.round(r.total_amount/1000)}k
                      </div>
                    </a>
                  </div>
                )
        ...
        <div style={{
          ...
          {[
            ...
            { icon:"??", label:"Ðon hàng",  key:"orders",   href:"/orders"   },
            { icon:"??", label:"Gi? hàng",  key:"cart",     href:"/cart",  cart:true },
            { icon:"??", label:"Cài d?t",   key:"settings", href:"/settings" },
          ].map(tab => (
            <button key={tab.key}
          ...
          <motion.div
            ...
            <motion.div
              ...
              <div style={{ color:"#b0956a", fontSize:12, textAlign:"center",
                ...
                Gi? hàng dang có món t?{" "}
                <span style={{ color:"#FF8C00", fontWeight:700 }}>{storeShopName}</span>.
                <br />Thêm món m?i s? <strong style={{ color:"#ff6060" }}>xóa gi? hàng hi?n t?i</strong>.
              </div>
