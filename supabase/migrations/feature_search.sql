...
-- Uses pg_trgm similarity + ILIKE so results survive even without pg_trgm index warm-up
...
BEGIN
  ...
  FROM (
    SELECT
      s.id, s.name, s.category, s.logo_url, s.rating_avg, s.is_open,
      GREATEST(similarity(s.name, query), similarity(s.category, query), 0) AS score
    FROM shops s
    WHERE s.status = 'approved'
      AND (
        s.name     ILIKE '%' || query || '%'
        OR s.category ILIKE '%' || query || '%'
        OR similarity(s.name,     query) > 0.15
        OR similarity(s.category, query) > 0.15
      )
    LIMIT 10
  ) r;
